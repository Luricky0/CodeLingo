...
function toInsertSQL(table: string, obj: Record<string, any>): string {
  const keys = Object.keys(obj);
  const values = keys.map((k) => {
    const v = obj[k];
    if (typeof v === "string") return `'${v.replace(/'/g, "''")}'`; // 转义单引号
    if (typeof v === "number") return v;
    if (v === null || v === undefined) return "NULL";
    return `'${JSON.stringify(v).replace(/'/g, "''")}'`; // 把对象/数组转成 JSON 字符串
  });
  return `INSERT INTO units (${keys.join(", ")}) VALUES (${values.join(", ")});`;
}

const sql = toInsertSQL("units", {
  id: unit1.id,
  chapter_id: unit1.chapterId, 
  order: unit1.order,
  title: unit1.title,
  questions: unit1.questions,
});

fs.writeFileSync("data.sql", sql);
console.log("✅ data.sql 已生成:\n", sql);